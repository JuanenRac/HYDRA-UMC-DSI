...
- **Aún falta una ejecución real en hardware Linux/CM5** - `flutter build linux`, `build_linux.sh` y la unidad de autoarranque `kiosk/hydra-umc-dsi.service` se han escrito y revisado, pero nunca se han ejecutado de verdad contra una máquina Linux real ni contra la propia CM5 desde este entorno de trabajo (una máquina Windows sin ninguna distribución WSL instalada). Quien las ejecute por primera vez debería tratarlo como la primera compilación/despliegue real de esta plataforma, no como un formalismo - ver la sección 7 de `docs/ARCHITECTURE.md` y "Ejecución en la CM5 real" más arriba.
