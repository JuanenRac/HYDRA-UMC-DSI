...
- **Ein echter Lauf auf Linux/CM5-Hardware steht noch aus** - `flutter build linux`, `build_linux.sh` und die Autostart-Unit `kiosk/hydra-umc-dsi.service` wurden geschrieben und durchgesehen, aber von dieser Arbeitsumgebung aus (einer Windows-Maschine ohne installierte WSL-Distribution) nie tatsächlich gegen eine echte Linux-Maschine oder das CM5 selbst ausgeführt. Wer sie zum ersten Mal ausführt, sollte das als den echten ersten Build/Deploy dieses Plattform-Ziels behandeln, nicht als Formalität - siehe Abschnitt 7 von `docs/ARCHITECTURE.md` und „Ausführen auf dem echten CM5" oben.
