...
- **Manca ancora un'esecuzione reale su hardware Linux/CM5** - `flutter build linux`, `build_linux.sh` e l'unità di avvio automatico `kiosk/hydra-umc-dsi.service` sono stati scritti e revisionati, ma mai eseguiti realmente contro una vera macchina Linux o il CM5 stesso da questo ambiente di lavoro (una macchina Windows senza alcuna distribuzione WSL installata). Chi li esegue per la prima volta dovrebbe considerarlo come la vera prima build/distribuzione di questa piattaforma, non una formalità - vedere la sezione 7 di `docs/ARCHITECTURE.md` e "Esecuzione sul CM5 reale" più sopra.
