...
**Correction to the record:** an earlier note that informed this
implementation described HYDRA-UMC-IOS-CONTROL as already using "a real
embedded Linux target." That was inaccurate - `HYDRA-UMC-IOS-CONTROL` has only
...
a KlipperScreen" the project owner asked for directly. Metrics is a new, dedicated tab (not present as its own screen on iOS/
