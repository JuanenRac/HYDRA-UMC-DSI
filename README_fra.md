...
- **Découverte réseau** (`lib/network/discovery.dart`) - balayage concurrent de `GET /api/hydra-info` sur les sous-réseaux locaux réels de cet appareil, repris sans modification depuis HYDRA-UMC-IOS-CONTROL.
...
- **Réglages** (`lib/ui/settings_screen.dart`) - informations de connexion, identité du serveur et déconnexion.
...
```bash
./build.sh          # Git Bash / WSL, ou build.bat pour cmd/PowerShell - flutter pub get + flutter build windows (vérification sur machine de développement)
./build_linux.sh    # Doit s'exécuter SUR une vraie machine Linux (ou le CM5 lui-même) - flutter build linux (la vraie cible de déploiement)
...
Une fois que `build_linux.sh` produit `build/linux/*/release/bundle/`, copiez tout le répertoire `bundle/` sur le CM5 (il dépend des fichiers `.so` situés à côté du binaire, pas seulement de l'exécutable lui-même) et exécutez directement le binaire qu'il contient. Pour un démarrage automatique façon kiosque (plein écran, sans décoration de gestionnaire de fenêtres, lancement au démarrage) - non encore implémenté dans ce dépôt, voir `mejoras_futuras.txt` - un compositeur kiosque Wayland minimal (par ex. `cage`) ou une unité systemd lançant directement ce binaire sur un framebuffer nu sont les deux approches les plus courantes pour ce type de déploiement sur écran tactile embarqué ; [`flutter-pi`](https://github.com/ardera/flutter-pi) (un intégrateur de moteur Flutter tiers pour Raspberry Pi, s'exécutant sans aucun système de fenêtrage) est une alternative plus légère à évaluer plus tard, mais il compile directement contre le moteur Flutter plutôt que de passer par `flutter build linux`, il nécessiterait donc sa propre étape de compilation séparée, ce n'est pas un remplacement direct de celle-ci.

## 📂 Structure du Dépôt

```text
HYDRA-UMC-DSI/
├── build.bat, build.sh              # flutter pub get + flutter build windows (vérification sur machine de développement)
├── build_linux.sh                   # flutter build linux (la vraie cible CM5 - à exécuter sur Linux réel)
...
│       ├── settings_screen.dart     # Informations de connexion + déconnexion
│       └── widgets/
...
Ce projet fait partie d'un écosystème robotique plus large du même auteur (JuanenRac / Electro Hobby 3D). À connaître, car une demande pourrait en réalité concerner l'un d'entre eux plutôt que ce dépôt :
...
- **[HYDRA-UMC-EDITOR-URDF](https://github.com/JuanenRac/HYDRA-UMC-EDITOR-URDF)** — créateur/éditeur graphique URDF de bureau (Python/PySide6) pour le catalogue de modèles de ce projet : récupère les fichiers sources depuis GitHub ou un dossier local, valide la faisabilité des degrés de liberté, édite couleur/échelle/cinématique avec aperçu 3D en direct, et transmet le résultat final à un serveur STUDIO en cours d'exécution. Réel et fonctionnel, pas un espace réservé.
- **HYDRA-UMC-DSI** *(ce dépôt)* — interface tactile native en Flutter pour l'écran tactile DSI 5"/7" propre à HYDRA-UMC (1280×720, même résolution dans les deux tailles) sur le Compute Module 5, contrôlant ce même serveur directement depuis la carte. Scaffold réel et fonctionnel avec les 6 écrans du catalogue connectés au serveur en direct ; compilation réelle de la cible Linux pas encore exécutée sur du matériel réel (environnement de travail uniquement Windows - voir la section « Compilation » du README).
...
La documentation (ce README et ses traductions - `README_spa.md`, `README_ita.md`, `README_fra.md`, `README_deu.md`) est disponible sous **Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)**. Texte complet sur https://creativecommons.org/licenses/by-sa/4.0/.
