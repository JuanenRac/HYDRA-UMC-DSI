...
- **Real Linux/CM5 hardware run still pending** - `flutter build linux`, `build_linux.sh`, and the `kiosk/hydra-umc-dsi.service` autostart unit have been written and reviewed but never actually run against a real Linux machine or the CM5 from this working environment (a Windows machine with no WSL distro installed). Whoever runs them for the first time should treat that as the real first build/deploy of this platform target, not a formality - see `docs/ARCHITECTURE.md` section 7 and "Running on the real CM5" above.
